...
const DB = (() => {
  ...
  const savePdf = async (id, name, arrayBuffer) => {
    ...
    // Save metadata locally so we don't need a Postgres table just for metadata
    // We will just use the file listing from the bucket.
    // However, to keep the `name` and `savedAt`, we can store metadata in localStorage 
    // or rely on Supabase custom metadata if available. Supabase doesn't easily return custom metadata on list()
    // So we'll store a quick map in localStorage to track real names.
